...
export class GameBoardComponent implements OnInit, AfterViewInit {
	...
	@Input()
	set incomingMove(incomingMove: IncomingMove) {
		if (incomingMove) {
	...
	onTileClick(tileIndex: number): void {
		...
		const coordinateDifferenceX = Math.abs(
			this.tileArray[tileIndex].coordinates.x - this.tileArray[emptySpaceTileIndex].coordinates.x
		);
		const coordinateDifferenceY = Math.abs(
			this.tileArray[tileIndex].coordinates.y - this.tileArray[emptySpaceTileIndex].coordinates.y
		);
		...
		this.currentPuzzle[currentPuzzleEmptySpaceIndex] = this.currentPuzzle[
			this.tileArray[tileIndex].currentPuzzleIndex
		];
